...
<div class="main-content app-content mt-0">
    <div class="side-app">
        ...
        <form action="{{ route('finance.piutang.receive-payment.update', $data_recieve->id) }}" method="POST" enctype="multipart/form-data" name="dynamic-form">
            @csrf
            @method('PUT')
            <!-- PAGE-HEADER -->
                            ...
                            <div class="row">
                                        ...
                                        <div class="d-flex d-inline">
                                            <select class="form-control select2 form-select"
                                                ...
                                                @foreach ($contact as $c)
                                                    <option value="{{ $c->id }}" {{ $c->id === $data_recieve->contact->id ? "selected" : "" }}>{{ $c->customer_name }}</option>
                                                @endforeach
                                            </select>
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input type="date" class="form-control" name="date_recieve" id="date_recieve" value="{{ $data_recieve->date_recieve }}"  >
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control select2 form-select"
                                            ...
                                            @foreach ($currencies as $c)
                                                <option value="{{ $c->id }}" {{ $c->id === $data_recieve->currency_id ? "selected" : "" }}>{{ $c->initial }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="no_transactions" id="no_transactions" class="form-control" readonly placeholder="Choose Transaction" value="{{ $data_recieve->transaction }}" />
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label for="" class="form-label">Description</label>
                                        <input type="text" class="form-control" name="description" id="description" value="{{ $data_recieve->description }}" placeholder="Desc"  >
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-6">
                                    <label class="custom-control custom-radio">
                                        <input type="checkbox" class="custom-control-input" id="choose_job_order" name="choose_job_order" value="{{ isset($data_recieve->job_order) ? 1 : 0 }}" {{ isset($data_recieve->job_order) ? "checked" : "" }}>
                                        <span class="custom-control-label"><b>Choose Job Order</b></span>
                                    </label>
                                </div>
                            </div>
                            <div style="{{ isset($data_recieve->job_order) ? "" : "display:none;" }}" id="job_order_display">
                                <div class="row">
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control select2 form-select"
                                                ...
                                                <option label="Choose One" selected disabled></option>
                                                @foreach ($marketing as $jo)
                                                    <option value="{{ $jo->id }}:{{ $jo->source }}" {{ $jo->id === $data_recieve->job_order_id && $jo->source === $data_recieve->source ? "selected" : "" }}>{{ $jo->job_order_id }} - {{ $jo->source }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                            ...
                            <div class="table-responsive">
                                <table class="table">
                                    ...
                                    <tbody id="form-container">
                                        @foreach($data_recieve->details as $data)
                                        <tr class="form-wrapper">
                                                    ...
                                                    <select class="form-control select2 form-select" data-placeholder="X" name="other_currency_type" onchange="getTotal(this)">
                                                        @foreach($exchange as $e)
                                                            @php
                                                                $name = '';
                                                                $dataExchange = 1;
                                                                if($e->from_currency_id === $data_recieve->currency_id) {
                                            ...
                                            <td>
                                                ...
                                                <div class="d-flex justify-content-between gap-2" style="{{ isset($data->dp_nominal) ? "" : "display: none !important" }}">
                                                    ...
                                                    <input type="text" hidden class="form-control" name="detail_dp_invoice_nominal" value="{{ $data->dp + $data->dp_receive }}"/>
                                                </div>
                                            </td>
                                            <td>
                            ...
                            <div class="row">
                                <div class="col-lg-6">
                                    <div class="form-group">
                                        ...
                                        <textarea name="note_recieve" id="note_recieve" cols="30" rows="5" placeholder="Tulis catatan transaksi di sini ( maks 1000 karakter )" class="form-control">{{ $data_recieve->note }}</textarea>
                                    </div>
                                </div>
                                <div class="col-lg-6">
                                    <table class="table mt-5">
                                        {{-- <tr>
                                            <td>
                                                <div class="d-flex justify-content-between align-items-center">
                                                    Biaya Lain
                                                    <input type="text" style="width: 50%" class="form-control" name="additional_cost" id="additional_cost" placeholder="0" value="{{ number_format($data_recieve->additional_cost,2,'.',',') }}" onchange="changeFormat(this)" />
                                                </div>
                                            </td>
                                        </tr> --}}
                                        <tr>
                                            <td>
                                                <div class="d-flex justify-content-between align-items-center">
                                                    Diskon
                                                    <input type="text" style="width: 50%" class="form-control" id="discount_display" name="discount_display" readonly placeholder="0" value="{{ number_format($data_recieve->discount,2,'.',',')}}" />
                                                </div>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                <div class="d-flex justify-content-between align-items-center">
                                                    Total
                                                    <input type="text" style="width: 50%" class="form-control" id="total_display" name="total_display" readonly placeholder="0" value="{{ number_format($data_recieve->total,2,'.',',')}}" />
                                                </div>
                                            </td>
                                        </tr>
                                        <tr style="{{ $data_recieve->dp > 0 ? "" : "display: none" }}" id="dp">
                                            <td>
                                                <div class="d-flex justify-content-between">
                                                    DP
                                                    <input type="text" style="width: 50%" class="form-control" id="display_dp" name="display_dp" readonly placeholder="0" value="{{ number_format($data_recieve->dp, 2, '.',',') }}" />
                                                </div>
                                            </td>
                                        </tr>
                                        ...
                                        <tr style="{{ $data_recieve->dp > 0 ? "block" : "display: none" }}" id="sisa">
                                            <td>
                                                <div class="d-flex justify-content-between">
                                                    Sisa
                                                    <input type="text" style="width: 50%" class="form-control" id="display_sisa" name="display_sisa" readonly placeholder="0"  value="{{ number_format($data_recieve->total - $data_recieve->dp, 2, '.',',')  }}"/>
                                                </div>
                                            </td>
                                        </tr>
                                    </table>
            ...
            $.ajax({
                ...
                success:function(response)
                {
                    if(response.message === "Success") {
                        const current_year = '{{ \Carbon\Carbon::parse($data_recieve->date_recieve)->year }}'
                        let number = '{{ $data_recieve->number }}'
