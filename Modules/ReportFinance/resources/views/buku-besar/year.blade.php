                ...
                <div class="row" style="background-color: white; padding-top: 50px; padding-bottom: 50px;" id="headerPrint">
                    <div class="col-xl-12">
                       <div class="w-full d-flex justify-content-center align-items-center flex-column">
                            ...
                            <p style="font-size: 18px; margin-top: -10px; font-weight: 500; color: #B14F4B;">{{ $year }}</p>
                       ...
                       <div class="d-flex gap-5 flex-column">
                           @foreach ($yearBukuBesar["data"] as $idx => $month)
                           @if(count($month) > 0)
                           <div>
                               <h1 style="font-size: 18px; margin-top: -10px; font-weight: 500; color: #B14F4B;">{{ $yearBukuBesar["month_name"][$idx] }}</h1>
                               <div style="font-size: 16px; font-weight: 400;">Semua Departemen</div>
                               <table class="table">
                                   <thead>
                                       <tr style="background-color: #597FB3;">
                                           <th style="width: 0px; color: white;" class="tableTh">#</th>
                                   ...
                                   @foreach ($month as $type)
                                        @php
                                            $filteredData = collect($type["data"])->filter(function ($data) {
                                                return $data["total_debit"] != 0 || $data["total_kredit"] != 0;
                                            });
                                        @endphp
                                       ...
                                       <tr style="background-color: #F0F6F9;">
                                           ...
                                           <td style="font-size: 16px; font-weight: 400;">({{$type["account_type"]->code}}) - {{ $type["account_type"]->name }}</td>
                                           <td></td>
                                           <td></td>
                                           <td></td>
                                           <td></td>
                                       ...
                                       @foreach ($filteredData as $data)
                                           <tr style="background-color: #FBFEFF;" class="detail">
                                               ...
                                               <td>
                                                   @php
                                                       $link = "#";
                                                       if($data["transaksi_id"]->transaction_type === "Saldo Awal") {
                                                           $link = route('finance.master-data.account');
                                                       } else if($data["transaksi_id"]->transaction_type === "Sales Order") {
                                                           $link = route('finance.piutang.sales-order.show', $data["transaksi"]->id);
                                                       } else if($data["transaksi_id"]->transaction_type === "Invoice") {
                                                       ...
                                                       } else if($data["transaksi_id"]->transaction_type === "Payment") {
                                                           $link = route('finance.payments.purchase-payment.show', $data["transaksi"]->id);
                                                       }
                                                   @endphp
                                                   <a href="{{ $link }}">
                                                       {{ $data["transaksi_id"]->transaction_type }}
                                                   </a>
                                               </td>
                                               <td>
                                               ...
                                               <td>
                                                ...
                                                @php
                                                    ...
                                                    $saldoString = number_format(abs($runningSaldo), 2, '.', ',');
                                                    if($runningSaldo < 0) {
                                                ...
                                                <td>
                                                    {{ 
                                                        round($runningSaldo, 2) == 0 ? "0.00" : 
                                                        ($runningSaldo < 0 ? "-".number_format(abs($runningSaldo),2,'.',',') : number_format($runningSaldo,2,'.',','))
                                                    }}
                                                </td>
                                            </tr>
                                       @endforeach
                                   ...
                                   <tr style="background-color: #597FB3;">
                                       ...
                                       <th style="color: white;"></th>
                                       <th style="color: white;"></th>
                                        <th style="color: white;">{{number_format($debitAkhir,2,'.',',')}}</th>
                                        <th style="color: white;">{{"-".number_format($KreditAkhir,2,'.',',')}}</th>
    ...
    <script>
        function addEventListeners() {
            document.getElementById("showExport").addEventListener("click", function() {
                ...
                var tableTh = document.querySelectorAll(".tableTh");
