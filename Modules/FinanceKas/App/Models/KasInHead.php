...
class KasInHead extends Model
{
    use HasFactory, HasRoles, SoftDeletes;
    protected $table = 'kas_in_head';
    protected $guarded = [];

    public function details()
    {
        return $this->hasMany(KasInDetail::class, 'head_id', 'id');
    }
    ...
    {
        $transaction_template = NoTransactionsKasIn::withTrashed()->find($this->transaction_id)->template;
        return "$transaction_template$this->number";
    }

    public function getTotalAttribute()
    {
        $details = KasInDetail::where('head_id', $this->id)->get();
        $total = 0;
