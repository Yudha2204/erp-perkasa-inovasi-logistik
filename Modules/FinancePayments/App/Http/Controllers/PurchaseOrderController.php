...
class PurchaseOrderController extends Controller
{
    ...
    public function create()
    {
        ...
        return view('financepayments::purchase-order.create', compact('vendor', 'currencies', 'terms', 'taxs', 'customer'));
    }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $ppn_masukan_id = MasterAccount::where('code', '110500')
                                ->where('account_name', 'PPN Masukan')
                                ->where('master_currency_id', $currency_id)->first();
        if(!$ppn_masukan_id) {
            return redirect()->back()->withErrors(['ppn_masukan' => 'Please add the account of PPN Masukan with code number is 110500']);
        }
        $ppn_masukan_id = $ppn_masukan_id->id;

        $biaya_produksi_id = MasterAccount::where('code', "550500")
                                ->where('account_name', 'Biaya Produksi')
                                ->where('master_currency_id', $currency_id)->first();
        if(!$biaya_produksi_id) {
            return redirect()->back()->withErrors(['biaya_produksi' => 'Please add the account of Biaya Produksi with code number is 550500']);
        }
        $biaya_produksi_id = $biaya_produksi_id->id;
        
        $beban_pokok_id = MasterAccount::where('code', "550000")
                                ->where('account_name', 'Beban Pokok Pendapatan')
                                ->where('master_currency_id', $currency_id)->first();
        if(!$beban_pokok_id) {
            return redirect()->back()->withErrors(['beban_pokok' => 'Please add the account of Beban Pokok Pendapatan with code number is 550000']);
        }
        $beban_pokok_id = $beban_pokok_id->id;
        ...
        OrderHead::create($data);
        $newestOrder = OrderHead::latest()->first();
        $head_id = $newestOrder->id;
        ...
        $flow = [
            //debit, kredit
            [0, $total_display, $hutang_usaha_id],
            [0, $discount_display, $diskon_pembelian_id],
            [0, $display_pajak, $ppn_masukan_id],
            [0, $display_dp, $kas_id],
            [$display_dp, 0, $dp_id],
            [$additional_cost, 0, $biaya_produksi_id],
            [$totBalance, 0, $beban_pokok_id]
        ];
