...
class FiscalPeriod extends Model
{
     ...
     * Note: This method will auto-create the period if it doesn't exist (for backward compatibility)
     *
     * @param string|\Carbon\Carbon $date
     * @return bool
     */
    ...
    /**
     * Check if a date falls within an open period (strict mode - period must exist)
     * Returns false if period doesn't exist
     *
     * @param string|\Carbon\Carbon $date
     * @return bool
     */
    public static function isDateInOpenPeriodStrict($date): bool
    {
        $date = Carbon::parse($date);
        $period = $date->format('Y-m');
        ...
        if (!$fiscalPeriod) {
            // Period doesn't exist - return false
            return false;
        }
    ...
    /**
     * Check if a fiscal period exists for the given date
     *
     * @param string|\Carbon\Carbon $date
     * @return bool
     */
    public static function periodExists($date): bool
    {
        $date = Carbon::parse($date);
        $period = $date->format('Y-m');
